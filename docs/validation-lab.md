...
ontoly diff base.graph head.graph --json
```

The diff is deterministic and defined by [RFC 0005](../rfcs/0005-graph-diffing.md).
It reports added, removed, and modified nodes and added or removed
edges. See [Graph Diffing](graph-diffing.md) for the full contract.

Coverage, trust, and performance deltas from validation-lab runs live
in the corpus dashboards and regression reports rather than the diff
output.

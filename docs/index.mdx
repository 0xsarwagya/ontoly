...
It ships three first-class language frontends — JavaScript / TypeScript (via
the TypeScript Compiler API), Python, and Go — and 29 framework analyzers
across all three. Every frontend produces the same node kinds, relationship
kinds, stable IDs, and query behavior over the same graph model.
...
- [JavaScript Support](/docs/javascript-support) - inspect ESM, CommonJS, JSX, and configuration behavior.
- [Python Support](/docs/python-support) - inspect the Python frontend and its framework analyzers.
- [Go Support](/docs/go-support) - inspect the Go frontend and its framework analyzers.
- [JavaScript and TypeScript Semantic Model](/docs/typescript-semantic-model) - inspect the pure language model.
- [Framework Analyzer API](/docs/framework-analyzer-api) - add deterministic framework analyzers for any frontend.
...
- [Agent Skills Catalog](/docs/skills-overview) - browse public docs pages for every installable Ontoly Skill.
...
- [Known Limitations](/docs/known-limitations) - what Ontoly v1.2.0 intentionally does not cover.
- [Compatibility Matrix](/docs/compatibility-matrix) - runtime, graph, skill, and MCP compatibility.
- [Feature Matrix](/docs/feature-matrix) - supported, experimental, and non-goal surfaces.
...
Stable. The JavaScript / TypeScript, Python, and Go frontends, the Software
Graph, Query Engine, MCP capabilities, Agent Skills, validation lab, and
benchmarks are in v1 freeze. Changes that affect the graph contract, public
APIs, packages, or skill compatibility still require the RFC and release-gate
process.

The current release is v1.2.0. See the [Version Matrix](/docs/version-matrix)
for package versions and the [Compatibility Matrix](/docs/compatibility-matrix)
for runtime and graph-spec compatibility.

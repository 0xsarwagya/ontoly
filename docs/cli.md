...
Local `build` and `output` commands keep an internal compiler snapshot in
`.ontoly/cache/compiler`. Ontoly fingerprints source contents, configuration,
pipeline version, compiler pass versions, and validation hooks. An unchanged
build reuses the validated graph and frontend products without parsing the
repository again.

```sh
ontoly build . --progress
ontoly build . --workers 8
ontoly build . --no-cache
```

`--workers` bounds independent work such as source hashing; it does not split a
single TypeScript program across files. JSON output includes cache status,
invalidation details, and per-stage timing. Changing a source reports exact
added, modified, and removed paths before the graph is rebuilt. Long-lived
watch and runner processes also reuse TypeScript's incremental builder.
...
`ontoly analyze` writes the JavaScript/TypeScript Semantic Model to

---
title: "Version Matrix"
description: "The Software Graph version is negotiated independently from Ontoly package versions."
canonical: "https://oss.sarwagya.wtf/ontoly/docs/version-matrix"
source: "docs/version-matrix.md"
keywords: ["Ontoly", "Software Graph", "TypeScript", "static analysis", "MCP", "AI coding agents", "developer tools", "Version", "Matrix"]
---

| Component | Version |
| --------- | ------- |
| Ontoly core packages | 1.0.0-rc.2 |
| Query Engine | 1.0.0-rc.2 |
| TypeScript analyzer | 1.0.0-rc.2 |
| TypeScript parser frontend | 1.0.0-rc.2 |
| Semantic Index | exported by `@0xsarwagya/ontoly-core` 1.0.0-rc.2 |
| Semantic Capability Engine | 1.0.0-rc.2 |
| Enhancer API | 1.0.0-rc.2 |
| Semantics Enhancer | 1.0.0-rc.2 |
| Intelligence API | 1.0.0-rc.2 |
| MCP runtime | 1.0.0-rc.2 |
| Ontoly CLI | 1.0.0-rc.2 |
| HTML graph plugin | 1.0.0-rc.2 |
| Software Graph Spec | 1.0 draft |
| Agent Skills | 1.0.0-rc.2 |
| Node.js engine | 20+ |
| pnpm engine | 10+ |
| TypeScript | 5.9 |

The Software Graph version is negotiated independently from Ontoly package versions.
